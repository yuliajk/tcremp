...
class TcrempPipelineVdjdb(TcrempPipeline):
    ...
    def tcremp_clonotypes(self, chain, unique_clonotypes=False):
        ...
        if chain == 'TRA' or chain == 'TRB':
            ...
            df = self.__clonotypes_data_clean(df, chain)  # 050624
            ...
            self.clonotypes_pred[chain] = self._TCRemP__clonotypes_prep(df, chain)  # 050624
            ...
            for current_chain in ['TRA', 'TRB']:
                df = self.__assign_clone_ids_with_vdjdb_paired(df, current_chain)
                df = self._TcrempPipeline__clonotypes_data_clean(df, current_chain)  # 050624
                self.clonotypes_pred[chain][current_chain] = self._TcrempPipeline__clonotypes_prep(df, current_chain)
                self.clonotypes_pred[chain][current_chain].to_csv(self.clonotypes_path[chain][current_chain], sep='\t')

                data_chain_1 = pd.concat([vdjdb, df])  # 050624

                self.clonotypes[chain][current_chain] = self._TcrempPipeline__clonotypes_prep(data_chain_1,
                                                                                              current_chain)
            ...
            for current_chain in ['TRA', 'TRB']:
                df = df[df[self.clonotype_id_dict[current_chain]].isin(
    ...
    def tcremp_dists(self, chain):
        ...
        if (chain == 'TRA') or (chain == 'TRB'):
            dists_vdjdb = self._TcrempPipeline__mir_results_proc(chain, self.vdjdb_dists_res_path[chain],
                                                                 self.vdjdb_clonotypes_path[chain], self.clonotype_id)

            dists_pred = self._TcrempPipeline__mir_results_proc(chain, self.dists_res_path[chain],
            ...
            for chain_1 in ['TRA', 'TRB']:
                dists_vdjdb = self._TcrempPipeline__mir_results_proc(chain_1, self.vdjdb_dists_res_path[chain][chain_1],
                                                                     self.vdjdb_clonotypes_path[chain][chain_1],
                                                                     self.clonotype_id)
                dists_pred = self._TcrempPipeline__mir_results_proc(chain_1, self.dists_res_path[chain][chain_1],
                                                                    self.clonotypes_path[chain][chain_1],
                                                                    self.clonotype_id)

                self.dists_dubs[chain][chain_1] = pd.concat([dists_vdjdb, dists_pred])
                self.dists[chain][chain_1] = self.dists_dubs[chain][chain_1].drop_duplicates(
                    self.clonotype_id).reset_index(
                    drop=True)
            ...
            def get_dists_for_chain(cur_chain):
                dists_data_res = dists_data.merge(self.dists[chain][cur_chain].rename(
                    {self.clonotype_id_dict[cur_chain]: self.clonotype_id_dict[chain][cur_chain]}, axis=1))
                return dists_data_res.drop(list(self.clonotype_id_dict[chain].values()), axis=1)

            dists_data_a = get_dists_for_chain('TRA')
            dists_data_b = get_dists_for_chain('TRB')

            self.dists[chain]['joined'] = dists_data_a.merge(dists_data_b, on=self.clonotype_id)
            self.annot_dists[chain] = self.dists[chain]['joined'].merge(annot_clones).drop(self.clonotype_id, axis=1)
...
class TcrempPipelinePred(TcrempClustering):
    ...
    def clstr_pred(self, chain, data, label_cl, model='dbscan', on_pca=True):
        ...
        self.clstr_labels[chain] = pd.merge(self.clstr_labels[chain],
                                            self.binom_res_train[chain].rename({
                                                label_cl: 'label_cluster_train',
                                                ...
                                                axis=1)[['cluster', 'label_cluster_train', 'fraction_matched_train',
                                                         'p_value_train', 'is_cluster_train', 'enriched_clstr_train']],
                                            on='cluster',
                                            how='left').sort_values(self.annotation_id).reset_index(drop=True)
        if chain == 'TRA_TRB':
