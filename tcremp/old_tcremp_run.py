...
tcr_columns_chain = {'TRA': ['a_cdr3aa', 'a_v', 'a_j'], 'TRB': ['b_cdr3aa', 'b_v', 'b_j'],
                     'TRA_TRB': ['a_cdr3aa', 'a_v', 'a_j', 'b_cdr3aa', 'b_v', 'b_j']}
label_cluster = 'label_cluster'

annotation_tcr_id_columns_dict = {'TRA': 'cloneId', 'TRB': 'cloneId',
...
def clustering(args, tcremp, outputs_path, output_columns):
    ...
    if args.label:
        df = tcremp.annot[args.chain][output_columns].merge(
            model.clstr_labels[args.chain][['cluster', label_cluster, tcremp.annotation_id]])
        ...
        df = tcremp.annot[args.chain][output_columns].merge(
            model.clstr_labels[args.chain][['cluster', tcremp.annotation_id]])
...
def main():
    ...
    tcremp = TCRemP.TCRemP(run_name=outputs_path, input_data=data_preped, clonotype_index=args.clonotype_index,
                           prototypes_path=args.prototypes_path, n=args.n, species=args.species,
                           prototypes_chain=args.chain, random_seed=args.random)
    ...
    tcremp.annot[args.chain][output_columns].merge(tcremp.annot_dists[args.chain]).to_csv(
        f'{outputs_path}tcremp_dists_{args.chain}.txt', sep='\t', index=False)

    ## pca
    print('Stage: PCA calculation')
    tcremp.tcremp_pca(args.chain)
    tcremp.annot[args.chain][output_columns].merge(tcremp.pca[args.chain]).to_csv(
        f'{outputs_path}tcremp_pca_{args.chain}.txt', sep='\t', index=False)

    ## tsne
    print('Stage: TSNE calculation')
    tcremp.tcremp_tsne(args.chain)
    tcremp.annot[args.chain][output_columns].merge(tcremp.tsne[args.chain]).to_csv(
        f'{outputs_path}tcremp_tsne_{args.chain}.txt', sep='\t', index=False)
