...
class TcrempClustering:
    ...
    def build_clusters(self, chain, data, label_cl=None, on_pca=True, knee_coef=None):
        ...
        if self.label[chain] is not None:
            self.binom_res[chain] = ml_utils.binominal_test(pd.merge(self.clstr_labels[chain], data.annot[chain]),
                                                            'cluster', label_cl, self.threshold)
            ...
            if -1 in list(self.clstr_labels[chain]['cluster']):
                self.binom_res[chain]['is_cluster'] = self.binom_res[chain].apply(
                    lambda x: x.is_cluster if x.cluster != -1 else 0, axis=1)
                self.binom_res[chain]['label_cluster'] = self.binom_res[chain].apply(
                    lambda x: x.label_cluster if x.cluster != -1 else 'no', axis=1)
        ...
        if chain == 'TRA_TRB':
            self.clstr_labels[chain] = pd.merge(self.clstr_labels[chain], data.annot[chain][[self.annotation_id]
                                                                                            + list(
                data.tcr_columns_paired['TRA'])
                                                                                            + list(
                data.tcr_columns_paired['TRB'])])
        else:
            self.clstr_labels[chain] = pd.merge(self.clstr_labels[chain], data.annot[chain][
                [self.annotation_id] + data.tcr_columns_paired[chain]])
    ...
    def clstr_metrics_calc(self, chain, data):
        ...
        self.clstr_metrics[chain]['total unique epitopes'] = str(
            len(data.annot[chain].drop_duplicates(self.label[chain])))
    ...
    def silhouette_clusters(self, X_data, chain):
        ...
        range_n_clusters = [
            ...
            , round(data_len * 0.4), round(data_len * 0.5), round(data_len * 0.6), round(data_len * 0.7),
            round(data_len * 0.8), round(data_len * 0.9)]

        silhouette_avg_scores = ml_utils.silhouette_avg_scores_kmeans(X_data, range_n_clusters)

        n_to_try = list(silhouette_avg_scores.keys())[
            list(silhouette_avg_scores.values()).index(max(silhouette_avg_scores.values()))]

        range_n_clusters = [round(n_to_try * 0.7), round(n_to_try * 0.8), round(n_to_try * 0.9), n_to_try,
                            round(n_to_try * 1.1), round(n_to_try * 1.2), round(n_to_try * 1.3)]
        silhouette_avg_scores = ml_utils.silhouette_avg_scores_kmeans(X_data, range_n_clusters)

        self.silhouette_n_clusters[chain] = list(silhouette_avg_scores.keys())[
            list(silhouette_avg_scores.values()).index(max(silhouette_avg_scores.values()))]
    ...
    def __is_cluster_by_between_metric(self, data, chain):
        ...
        br['pred_enriched_min_between'] = br.apply(lambda r: 1 if (r.is_cluster == 1) & (r.dist <= try_dist) else 0,
                                                   axis=1)

        pred_enriched_between_cls = list(br[br['pred_enriched_min_between'] == 1]['cluster'])

        self.clstr_labels[chain]['is_cluster_between'] = self.clstr_labels[chain]['cluster'].apply(
            lambda x: 1 if x in pred_enriched_between_cls else 0)
    ...
    def __plot_logo(self, clstr_data, chain, c, list_ax, tcr_columns_paired):
        ...
        for l in lengs:
            ...
            if len(seqs) >= 2:
                ...
                list_ax[0].set_title(
                    f"{chain}. Cluster: {c} {epi}\nFraction matched:{round(fr_matched, 2)}\nCount of cdr3aa: {len(seqs)}")
        plot_v_j = clstr_data[clstr_data['cluster'] == c]

        plot_v_j = pd.DataFrame(
            plot_v_j.groupby(tcr_columns_paired[chain][1])[tcr_columns_paired[chain][0]].count().reset_index())
        list_ax[1].pie(plot_v_j[tcr_columns_paired[chain][0]], labels=plot_v_j[tcr_columns_paired[chain][1]])

        plot_v_j = clstr_data[clstr_data['cluster'] == c]
        plot_v_j = pd.DataFrame(
            plot_v_j.groupby(tcr_columns_paired[chain][2])[tcr_columns_paired[chain][0]].count().reset_index())
